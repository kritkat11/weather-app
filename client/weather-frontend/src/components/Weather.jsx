...
function Weather() {
    ...
    const getWeatherByLocation = async (lat, lon) => {
        try {
            ...
            const response = await axios.get(`http://localhost:1234/api/weather?lat=${lat}&lon=${lon}`);
    ...
    const handleSearch = async () => {
        ...
        try {
            const response = await axios.get(`http://localhost:1234/api/weather?city=${city}`);
